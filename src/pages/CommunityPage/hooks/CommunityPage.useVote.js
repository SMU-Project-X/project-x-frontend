...
export const useVote = () => {
    ...
    const fetchVotes = async (bannerId) => {
        try {
            const { data: voteResult } = await axios.get(`http://localhost:8080/api/vote/voteResult/${bannerId}`);
            console.log("투표결과: ", voteResult);

            const { data: voteDetail } = await axios.get(`http://localhost:8080/api/vote/${bannerId}`);
            console.log("투표 상세: ", voteDetail);
        } catch (err) {
